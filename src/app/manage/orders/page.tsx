    ...
    <div className="space-y-12">
      <div className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-1.5 bg-white border border-parchment px-5 py-2 rounded-full shadow-sm">
           ...
           <span className="text-[0.65rem] font-bold uppercase tracking-widest text-warm">Queue: {orders.length}</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-parchment rounded-[2.5rem] overflow-hidden shadow-sm">
        {loading ? (
          <div className="py-32 flex flex-col items-center gap-4 text-warm opacity-40">
             ...
             <p className="text-[0.7rem] uppercase tracking-[0.3em] font-black">Syncing Order Ledger...</p>
          </div>
        ) : orders.length === 0 ? (
          <div className="py-32 text-center bg-cream/20 border-dashed border border-parchment rounded-[2.5rem] m-8">
             <p className="font-serif italic text-xl text-warm opacity-60">The order book is currently breathing...</p>
          </div>
        ) : (
                    ...
                    >
                      ...
                      <td className="py-6">
                        <div className="flex flex-col">
                          <span className="text-[0.9rem] font-medium text-charcoal">{order.user_name}</span>
                          <span className="text-[0.65rem] text-warm opacity-60 italic">{order.user_email}</span>
                        </div>
                      </td>
                      ...
                      <td className="py-6">
                        <select
                          ...
                          className={`text-[0.6rem] font-black uppercase tracking-widest px-4 py-1.5 rounded-full border cursor-pointer outline-none shadow-sm transition-all appearance-none text-center ${STATUS_STYLING[order.status] || ""}`}
                        >
                          {STATUS_OPTIONS.map(s => <option key={s} value={s} className="bg-white text-charcoal capitalize">{s}</option>)}
                        </select>
                      </td>
                      <td className="py-6">
                      ...
                      <td className="py-6 pr-8 text-right">
                        <ChevronDown className={`w-4 h-4 text-parchment group-hover:text-sage-dark transition-all ${expanded === order.id ? "rotate-180 text-sage-dark" : ""}`} />
                      </td>
                    </tr>
                    {expanded === order.id && (
                          ...
                          <div className="grid md:grid-cols-2 gap-12 relative">
                            ...
                            <div className="space-y-6">
                               ...
                               <div className="bg-white/60 border border-parchment/60 rounded-[1.5rem] p-6 space-y-3">
                                  ...
                                  <div className="space-y-1 text-[0.85rem] text-warm leading-snug">
                                     ...
                                     <p className="font-medium uppercase tracking-[0.05em]">{order.address?.city}, {order.address?.state} — {order.address?.pincode}</p>
                                  </div>
                               ...
                               {order.notes && (
                                <div className="p-4 bg-orange-50 border border-orange-100 rounded-xl">
                                  <p className="text-[0.65rem] uppercase font-black text-orange-800 tracking-wider mb-2">Customer Directives</p>
                                  <p className="text-[0.82rem] text-orange-700 italic font-medium leading-relaxed">&quot;{order.notes}&quot;</p>
                                </div>
                               )}
                            </div>
                            ...
                            <div className="space-y-6">
                               <div className="flex items-center gap-2 mb-2">
                                  ...
                                  <h4 className="text-[0.7rem] uppercase font-black tracking-widest text-charcoal">Package Manifest</h4>
                               </div>
                               <div className="space-y-4 max-h-60 overflow-y-auto pr-4 custom-scrollbar">
                                  {Array.isArray(order.items) && order.items.map((item, i) => (
                                    <div key={i} className="flex justify-between items-center group/item">
                                       <div className="flex flex-col">
                                          <span className="text-[0.92rem] font-serif text-charcoal">{item.name}</span>
                                          <span className="text-[0.62rem] uppercase tracking-widest text-warm font-bold">{item.variant} <em className="opacity-40 mx-1">/</em> {item.size} <em className="opacity-40 mx-1">/</em> ×{item.qty}</span>
                                       </div>
                                       <div className="text-right">
                                          <span className="font-serif text-[1rem] text-charcoal">{formatPrice(item.price * item.qty)}</span>
                                          <span className="block text-[0.6rem] text-warm italic opacity-60">@{formatPrice(item.price)} ea.</span>
                                       </div>
                                    </div>
                                  ))}
                               </div>
                               <div className="pt-6 border-t border-parchment border-dashed flex justify-between items-end">
                                  <div className="flex flex-col">
                                     <span className="text-[0.65rem] uppercase tracking-widest font-black text-sage-dark mb-1">Total Valuation</span>
                                     <span className="text-[0.75rem] text-warm font-medium uppercase">Incl. Shipping & Taxes</span>
                                  </div>
